...
/*
  Data structure for temporary storage. This is needed because the size
  of the final data structures is only known after the whole input file
  has been read.
*/
struct rwlstruct{
  int first_yr;
...
SEXP rcompact(SEXP filename){
  char field_id, *line, *found1, *found2, *found_leftpar, *found_dot,
    *found_rightpar, *id_start, *old_point, *point, *endp, *tmp_name,
    *mplier_str;
  ...
  /* The file is read to this buffer one line at a time */
  line = (char *) R_alloc(LINE_LENGTH, sizeof(char));
  /* Engineering notation representation of the multiplier or divisor */
  mplier_str = (char *) R_alloc(MPLIER_LENGTH, sizeof(char));
  /* The first rwlnode */
  first = (rwlnode *) R_alloc(1, sizeof(rwlnode));
  ...
  while(fgets_eol(line, &n_content, LINE_LENGTH, f)){
    /* A simple check to point out too long header lines.
       Generally, if one line is too long, this function will probably
       be unable to parse the next line. In that case, finding the
       faulty line may be of some value. Of course, if the input is
       generated by some program, lines are expected to be short
       enough. Data edited by hand may be a different case.
    */
    ...
    if(!found1){
      fclose(f);
      error("Series %d: No '=' found when header line was expected", n+1);
    }
    if(found1 == line){
      fclose(f);
    ...
    if(read_int > INT_MAX){
      fclose(f);
      error("Series %d: Number %lld exceeds integer range", n+1, read_int);
    }
	...
	error("Series %d: Length of series must be at least one (%ld seen)",
      ...
      fclose(f);
      error("Series %d: Unknown or doubled field id: %c", n+1, *(found2+1));
    }
    ...
    if(snprintf(mplier_str, MPLIER_LENGTH, "1e%d", exponent) >= MPLIER_LENGTH){
      fclose(f);
      error("Series %d (%s): Exponent has too many characters",n+1,this->id);
    }
    if(*endp != '('){
      fclose(f);
    ...
    if(!found_rightpar){
      fclose(f);
      error("Series %d (%s): No closing parenthesis found", n+1, this->id);
    }
    if(*(found_rightpar+1) != '~'){
      fclose(f);
	...
	*point = '\0'; /* overwrite is OK because number has been read */
	...
	/*
	  Division by a precise number (integer value) is more accurate
	  than multiplication with an approximate number. Example from R:
