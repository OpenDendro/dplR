series.rwl.plot <- function(rwl,series,series.yrs=as.numeric(names(series)),
  ...
  legend(min(yrs,na.rm=T), y = max(series,master,na.rm=T),
        legend = c('Detrended Series','Detrended Master'), col = c(col.pal[1],col.pal[2]),
  ...
  plot(yrs,series,type="n", ylim=c(-1,1), ylab="",xlab="Year",
    sub=paste('Segments: length=',seg.length,',lag=',seg.lag,',bin.floor=',bin.floor,sep=''),
    axes=FALSE)
  abline(v=bins,col='grey',lty='dotted')
  abline(h=0)
  axis(1,at=bins[seq(from=1, to=nbins, by=2),])
  ...
  txt1 <- paste("Series:",min(na.omit(series.yrs0)),"-",max(na.omit(series.yrs0)),sep="")
  text(-1,1,txt1,pos=4)
  txt2 <- paste("Master:",min(yrs0,na.rm=TRUE),"-",max(yrs0,na.rm=TRUE),sep="")
