## The Tucson reader that shipped in dplR through version 1.8.0, kept under
## .legacy so that scripts depending on its exact output have somewhere to go.
## dplR 1.8.0 replaced read.tucson() with a new implementation; see
## ?read.tucson for what changed and ?read.tucson.legacy for why you might
## still want this one.  Naming follows glk.legacy() and rwi.stats.legacy().
`read.tucson.legacy` <- function(fname, header = NULL, long = FALSE,
