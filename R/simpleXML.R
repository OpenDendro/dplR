...
### Another possible choice: \x0A, i.e. just the line feed.
  ...
  to.xml.utf8 <- function(x){
    ...
    if(enc == "unknown"){
      x <- iconv(x, from="", to="UTF-8")  # from encoding of the locale
    } else if(enc == "latin1"){
      x <- iconv(x, from=enc, to="UTF-8") # from latin1
    }
    x.int <- utf8ToInt(x)
    ...
    good.chars <-
      (x.int == 0x9 |
       x.int == 0xA |
       x.int == 0xD |
       (x.int >= 0x20 & x.int <= 0xD7FF) |
       (x.int >= 0xE000 & x.int <= 0xFFFD) |
       (x.int >= 0x10000 & x.int <= 0x10FFFF))
    x <- paste(unlist(strsplit(x, split="", fixed=TRUE))[good.chars],
               collapse = "")
  }
       ...
       addTag = function(tag, content=NULL, attrs=character(0), close=TRUE){
         ...
         if(close){
           if(length(content) == 0){
             ...
             cat(">",
                 escape.content(to.xml.utf8(paste(content, collapse=" "))),
                 "</", tag, ">", line.term,
                 sep="", file=f)
           }
         } else{
       ...
       addTag.noCheck = function(tag, content=NULL, attrs=character(0),
         ...
         if(close){
           if(length(content) == 0){
             ...
             cat(">", content, "</", tag, ">", line.term, sep="", file=f)
           }
         } else{
           cat(">", line.term, sep="", file=f)
           indent <<- indent + indent.step
